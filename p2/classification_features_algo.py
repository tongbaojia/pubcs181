...
#practice, play around with training data split into training and testing datasets
    ffs = [first_last_system_call_feats, system_call_count_feats]
      
    X_train1,global_feat_dict1,t_train1,train_ids1 = extract_feats(ffs, "train1")
    X_train2,_,t_ignore,test_ids = extract_feats(ffs, "train2", global_feat_dict=global_feat_dict1)
    
    #logistic regression
    log_reg = LogisticRegression(penalty='l2', tol=0.0001, C=1.0,
     fit_intercept=True, class_weight=None, solver='liblinear', max_iter=100,
     multi_class='ovr', n_jobs=-1).fit(X_train1, t_train1)
    pred = log_reg.predict(X_train2)
    accuracy = np.sum(t_ignore == pred)/float(t_ignore.size)
    print(accuracy)
    
    #regression tree
    mod_tree = tree.DecisionTreeClassifier().fit(X_train1,t_train1)
    pred = mod_tree.predict(X_train2)
    accuracy = np.sum(t_ignore == pred)/float(t_ignore.size)
    print(accuracy)
    
    #random forest
    rf = RandomForestClassifier(n_estimators=500).fit(X_train1, t_train1)
    pred = rf.predict(X_train2)
    accuracy = np.sum(t_ignore == pred)/float(t_ignore.size)
    print(accuracy)
    
    #svm with kernel polynomial features degree 2
    mod_svm = svm.SVC(C=1.0, kernel='rbf', degree=2, decision_function_shape = 'ovr').fit(X_train1, t_train1)
    pred = mod_svm.predict(X_train2)
    accuracy = np.sum(t_ignore == pred)/float(t_ignore.size)
    print(accuracy)
    
    #K nearest neighbors
    knn = KNeighborsClassifier(n_neighbors=1).fit(X_train1, t_train1)
    pred = knn.predict(X_train2)
    accuracy = np.sum(t_ignore == pred)/float(t_ignore.size)
    print(accuracy)
    
    #Naive Bayes
    nb = GaussianNB().fit(X_train1.toarray(), t_train1)
    pred = nb.predict(X_train2.toarray())
    accuracy = np.sum(t_ignore == pred)/float(t_ignore.size)
    print(accuracy)
    nb2 = MultinomialNB().fit(X_train1, t_train1)
    pred = nb2.predict(X_train2)
    accuracy = np.sum(t_ignore == pred)/float(t_ignore.size)
    print(accuracy)
    
    #nnet
    nnet = MLPClassifier(hidden_layer_sizes=(3,5,3), max_iter=10000).fit(X_train1, t_train1)
    pred = nnet.predict(X_train2)
    accuracy = np.sum(t_ignore == pred)/float(t_ignore.size)
    print(accuracy)
    
    #adaboost
    adaboost = AdaBoostClassifier(DecisionTreeClassifier(max_depth=2), n_estimators=600, learning_rate=1.5, algorithm="SAMME").fit(X_train1, t_train1)
    pred = adaboost.predict(X_train2)
    accuracy = np.sum(t_ignore == pred)/float(t_ignore.size)
    print(accuracy)
    
    #gradient boosting
    gb = GradientBoostingClassifier(loss='deviance', learning_rate=0.1, n_estimators=100, max_depth=3).fit(X_train1.toarray(), t_train1)
    pred = gb.predict(X_train2.toarray())
    accuracy = np.sum(t_ignore == pred)/float(t_ignore.size)
    print(accuracy)
...
def main():
    ...
    print "learning..."
    #rf = RandomForestClassifier(n_estimators=500).fit(X_train, t_train)
    gb = GradientBoostingClassifier(loss='deviance', learning_rate=0.1, n_estimators=100, max_depth=3).fit(X_train.toarray(), t_train)
    print "done learning"
    print
    ...
    #random forest on total data, for submission purposes
    #preds = rf.predict(X_test)
    preds = gb.predict(X_test.toarray())
