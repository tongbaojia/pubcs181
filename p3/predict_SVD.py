...
with open(artist_file, 'r') as artist_fh:
    ...
    for row in artist_csv:
        ...
        if not artist in artist_num:
            ...
            count+=1
print('Forming sparce matrices')
import scipy
from scipy.sparse import *
train_mat = np.zeros((len(user_num),len(artist_num)), dtype=np.float)
...
#now fill in user medians as a starting point for the prediction
...
for i in range(k_vals.size):
    ...
    test_mat = np.zeros((len(user_num),len(artist_num)), dtype=np.float)
    pred_mat = np.zeros((len(user_num),len(artist_num)), dtype=np.float)
    ...
    for user in test_data:
        ...
        for artist in test_data[user]:
            ...
            pred_mat[user_id, artist_id] = np.matmul(np.matmul(U[user_id,:], np.diag(S)), V[:,artist_id])*np.float(user_total[user])
    ...
    print('Num svs = '+str(k_vals[i])+' mean error '+str(acc[i]))
"""
import matplotlib.pyplot as plt
plt.clf()
fig = plt.figure()
plt.plot(k_vals[0:4],acc[0:4])
plt.xlabel('Number of singular values')
plt.ylabel('Mean absolute error')
plt.show()
fig.savefig('SVD_k_100-400.png')
"""